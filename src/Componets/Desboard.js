...
const DashboardUI = () => {
  ...
  useEffect(() => {
    fetch('https://backend-6hdo.onrender.com/Ohkla/companies')
      .then(res => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`https://backend-6hdo.onrender.com/Ohkla/payment-years?MembershipID=${selectedMembershipID}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSearch = () => {
    ...
    const url = isAll || selectedYears.length > 1
      ? 'https://backend-6hdo.onrender.com/Ohkla/dashboard-multiyear'
      : 'https://backend-6hdo.onrender.com/Ohkla/dashboard-data';
    ...
    fetch(url, {
      ...
      .then(data => {
        ...
        fetch(`https://backend-6hdo.onrender.com/Ohkla/other-payments?companyName=${company}`)
          .then(res => res.json())
          .then(op => {
